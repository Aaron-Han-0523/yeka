...
class ProductRepo {
  ...
  Future<ApiResponse> updateProduct(ProductModel productModel) async {
    ...
    try {
      final response =
          await dioClient.put(AppConstants.UPDATE_AUTO_URI + "/${productModel.id}", data: _data);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getProductList(int limit, int skip) async {
    try {
  ...
  Future<ApiResponse> getProduct(ProductModel productModel) async {
    try {
      final response = await dioClient.get(
        AppConstants.GET_AUTO_URI + "/${productModel.id}",
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
